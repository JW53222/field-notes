...
Plenty was wholesale recycled — subagents are out-of-the-box Claude Code; the search algorithms and most of the math come straight from libraries and published literature, pulled in deliberately after a written competitive survey of ten platforms. *(Corrected 07-16-2026, after auditing my own transcripts: "pulled in after the survey" flatters the timeline. The López de Prado overfitting canon entered the codebase in March — Claude supplied it from training when I asked, in plain English, how to keep the discovery engine from fooling itself — and the July survey then went to the open web and proudly "discovered" methods my repo had been running for four months. The sessions didn't know what the codebase already contained, which is the best argument for the context-injection system anywhere in this post, delivered by my own logs.)*

But the systems I'd defend in any design review were problem-built, and the record shows the steering. The context-injection system — the thing that tells every agent what a file means the moment it touches one, down to per-function granularity — grew in steps I pushed by hand across months of sessions: an agent hand-wrote link docs, then I added a RAG index, then I discovered AST (and dropped the agent written docs), then I interwove the two feeds, then I worked out injection — and "worked out" hides the best part. The first version was a lookup: the map sat there, clearly available, for any agent to query. The improvement underwhelmed me, so I did what you do with any workforce and asked the agents how the tooling was going. They admitted they were ignoring it. So I stopped expecting them to remember a tool and moved to push: a scoped pre-tool-use hook that injects the pre-baked context for whatever file an agent touches, unasked. A couple of them then got creative and started writing *new* code instead of touching existing files — dodging the trigger — so the hook grew a post-edit twin. Every step of that is ordinary user research; the users just happen to be Claudes. Today that index weighs 42 megabytes and covers 2,948 files, 99,770 symbols, and 15,705 function-level embedding vectors, delivered by a hook the moment an agent touches a file. One of its ancestors was twenty Haiku agents reading the codebase in partitions and writing down what they saw — for six weeks the repo's self-knowledge was a small literate workforce, until three hundred lines of parser replaced them. Claude executed each step well once I pushed it to the answer, and never once volunteered the next step or named the industry pattern. Nobody in any session ever said the words for what vendors sell this as. I found out afterward it has a name and a price per seat.

*(Provenance status, added 07-16-2026: I audited this story against my session archives. The bracketing steps have commits — the lookup tooling, `7547187f8` flipping pull to push, `e6f2ef7a7` adding the write-time twin. The "they admitted they were ignoring it" exchange falls in a window whose transcripts weren't retained: that beat is memory, stated as such. The "nobody ever named it" claim, though, got checked rather than asserted — a full-corpus grep for the vendor vocabulary comes back zero hits.)*
