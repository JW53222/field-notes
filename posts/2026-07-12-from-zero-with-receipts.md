...
Plenty was wholesale recycled — subagents are out-of-the-box Claude Code; the search algorithms and most of the math come straight from libraries and published literature, pulled in deliberately after a written competitive survey of ten platforms. *(Corrected 07-16-2026, per the transcript and git record: the López de Prado overfitting canon first entered the codebase in March (`1d2e11a82`) — Claude supplied it from training when I asked, in plain English, how to keep the discovery engine from fooling itself — and the July survey independently re-derived it from the open web, citations attached. Sessions don't know what the codebase already contains; that's the strongest argument for the context-injection system anywhere in this post.)*
